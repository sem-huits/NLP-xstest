...
def build_sample():
    ...
    for model in MODELS:
        ...
        sample = (
            safe.groupby("type", group_keys=False)
                .apply(lambda x: x.sample(
                    min(len(x), max(1, SAMPLE_SIZE // safe["type"].nunique())),
...
class AnnotationApp:
    ...
    def __init__(self, root, df, output_path):
        ...
        if os.path.exists(output_path):
            ...
            if "annotation" in saved.columns:
                self.df["annotation"] = saved["annotation"].values
    ...
    def _load_item(self):
        ...
        # highlight already-annotated items
...
def main():
    ...
    if os.path.exists(output_path):
        ...
        print(f"Resuming from {output_path} ({(df['annotation'] != '').sum()}/{len(df)} done)")
    else:
